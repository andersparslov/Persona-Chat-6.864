...
class Bert2Bert(TorchGeneratorModel):
    """Sequence to sequence parent module."""

    
    def __init__(
        self, dropout=0, share_weight=False, attention_length=48,
    ):
        """
        Initialize bert2bert model
        """
        self.bert2bert = EncoderDecoderModel.from_encoder_decoder_pretrained("bert-base-cased", 
                                                                             "bert-base-cased", 
                                                                             tie_encoder_decoder=share_weight)
        ...
        self.bert2bert.config.max_length = attention_length
        self.bert2bert.config.min_length = 1
        ...
        self.hidden_size = self.bert2bert.config.hidden_size
        ...
        self.output = OutputLayer(
            num_features, embeddingsize, self.hidden_size, dropout=dropout,
...
class TransformerEncoder(nn.Module):
    ...
    def __init__(self, lm, dropout=0.1):
        """Initialize encoder."""
        super().__init__()
        self.lm = lm
        self.tokenizer = transformers.AutoTokenizer.from_pretrained('distilbert-base-cased')
